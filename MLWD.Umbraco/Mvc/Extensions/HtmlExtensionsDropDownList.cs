...
namespace MLWD.Umbraco.Mvc.Extensions
{
    ...
    public static class HtmlExtensionsDropDownList
    {
        ...
        public static IList<GroupedSelectListItem> ToGroupedSelectListItemList<T>(this IEnumerable<T> items, Func<T, string> text, Func<T, string> value, Func<T, string> key)
        {
            var list = new List<GroupedSelectListItem>();
            foreach (var item in items)
            {
                list.Add(new GroupedSelectListItem() { Text = text(item), Value = value(item), GroupKey = key(item) });
            }
