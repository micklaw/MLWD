...
namespace Vega.USiteBuilder
{
    ...
    public class ManagerBase
    {
        ...
        {
            var documentTypeAttribute = DocumentTypeManager.GetNodeTypeAttribute(typeContentType);
            ...
            foreach (PropertyInfo propInfo in typeContentType.GetProperties(BindingFlags.DeclaredOnly | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance))
            {
                ...
                string tabName = propAttr.GetTabAsString();

                if (string.IsNullOrEmpty(tabName))
                {
                    tabName = Util.GetAttribute<ContentTypeAttribute>(typeContentType).DefaultTab;
                }
                ...
                if (!string.IsNullOrEmpty(tabName) && tabName.ToLower() != DocumentTypeDefaultValues.TabGenericProperties.ToLower())
                {
                    ...
                    if (propAttr.GetTabOrder().HasValue)
                    {
                        contentType.SetTabSortOrder(tabId, propAttr.GetTabOrder().Value);
                    }
        ...
        public static DataTypeDefinition GetDataTypeDefinition(Type typeContentType, ContentTypePropertyAttribute propAttr,
                                                                PropertyInfo propInfo)
        {
            ...
            if (propAttr.Type == null)
            {
                ...
                {
                    return DataTypeDefinition.GetDataTypeDefinition((int)ContentTypePropertyUI.Textstring);
                }
                if (propInfo.PropertyType == typeof(DateTime))
                {
                    return DataTypeDefinition.GetDataTypeDefinition((int)ContentTypePropertyUI.DatePicker);
                }
                if (propInfo.PropertyType == typeof(bool))
                {
                    return DataTypeDefinition.GetDataTypeDefinition((int)ContentTypePropertyUI.TrueFalse);
                }
                if (propInfo.PropertyType == typeof(int) || propInfo.PropertyType == typeof(decimal) || propInfo.PropertyType == typeof(double))
                {
                    return DataTypeDefinition.GetDataTypeDefinition((int)ContentTypePropertyUI.Numeric);
                }
                if (propInfo.PropertyType == typeof(HtmlString))
                {
                    return DataTypeDefinition.GetDataTypeDefinition((int)ContentTypePropertyUI.RichtextEditor);
                }
            }

            if (propAttr.Type == ContentTypePropertyUI.Other)
            {
