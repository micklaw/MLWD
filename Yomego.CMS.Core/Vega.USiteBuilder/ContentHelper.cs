...
namespace Vega.USiteBuilder
{
    ...
    public static class ContentHelper
    {
        ...
        /// </summary>
        /// <param name="parentId">Parent node id of all children to get</param>
        public static List<Content> GetChildren(int parentId)
        {
        ...
        /// <returns></returns>
        public static List<Content> GetChildren(int parentId, bool deepGet)
        {
            List<Content> retVal = new List<Content>();
        ...
        public static T GetByNode<T>(Node node)
             where T : Content, new()
        {
            Content retVal = ContentHelper.GetByNode(node);
        ...
        /// <param name="nodeId">Node Id associated with the content item</param>
        /// <returns>Content item</returns>
        public static Content GetByNodeId(int nodeId)
        {
            Node node = new Node(nodeId);
        ...
        /// <param name="node">node associated with the content item</param>
        /// <returns>Content item</returns>
        public static Content GetByNode(Node node)
        {
            ...
            if (typeDocType != null)
            {
                ...
                {
                    retVal = (Content)Activator.CreateInstance(typeDocType);
                }

                else
                {
                    retVal = (Content)constructorInfo.Invoke(new object[] { node.Id });
                }
                retVal.Id = node.Id;
                retVal.ContentTypeAlias = node.NodeTypeAlias;
                retVal.Name = node.Name;
                retVal.ParentId = node.Parent.Id;
                ...
                foreach (PropertyInfo propInfo in typeDocType.GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance))
                {
                    ...
                    try
                    {
                        ...
                        else if (IsType<IList<string>>(propInfo.PropertyType))
                        {
                            value = StringUtils.SplitOnNewLines(property.Value ?? String.Empty);
                        }
                        else if (IsType<IList<int>>(propInfo.PropertyType))
                        {
                            var list = StringUtils.SplitOnCommonChars(property.Value ?? String.Empty);

                            value = list.Select(n => Convert.ToInt32(n)).ToList();
                        }
        ...
        private static bool IsType<T1>(Type type)
        {
        ...
        public static List<Content> SelectContentNodes(string xpath)
        {
            List<Content> retVal = new List<Content>();
        ...
        //    if (contentItem.Id == 0 && contentItem.Parent == null)
        //    {
        ...
        public static bool IsInRecycleBin(Content contentItem)
        {
