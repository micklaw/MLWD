...
namespace Yomego.CMS.Core.Utils
{
    public class UrlUtils
    {
        ...
        public UrlUtils(Uri uri)
            : this(uri.PathAndQuery)
        {

        }

        public UrlUtils(HttpRequest request)
            : this(request.Url)
        {
        ...
        public Uri Uri
        {
            get
            {
                if (_uri == null)
                {
                    _uri = HttpContext.Current.Request.Url;
                }
        ...
        public string SiteUrl()
        {
            ...
            if (this.Uri != null)
            {
                returnURL = string.Format("{0}://{1}/", this.Uri.Scheme,
                                                        this.Uri.Host + (IsLocalhost() ? ":" + this.Uri.Port.ToString() : string.Empty));
            }

            return returnURL;
        }

        public string SiteUrlFromHeaders()
        {
            string returnURL = string.Empty;
        ...
        {
            return (this.Uri.Host.ToLower().Equals("localhost") || this.Uri.Host.ToLower().Equals("127.0.0.1"));
        }
    }
}
