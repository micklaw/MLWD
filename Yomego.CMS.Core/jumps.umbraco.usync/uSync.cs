...
namespace jumps.umbraco.usync
{
    ...
    public class uSync : IApplicationEventHandler // works with 4.11.4/5 
    {
        ...
        public void DoSync(RunMode mode, bool? read = null, bool? write = null, bool? attach = null)
        {
            // everything we do here is blocking
            // on application start, so we should be 
            // quick. 
            GetSettings(mode, read, write, attach);

            RunSync();
        }

        private void GetSettings(RunMode mode, bool? read = null, bool? write = null, bool? attach = null)
        {
            try
            {
                LogHelper.Debug<uSync>("Get Settings: {0}", () => mode);

                _read = read ?? uSyncSettings.Read;
                LogHelper.Debug<uSync>("Settings : Read = {0}", () => _read);

                _write = write ?? uSyncSettings.Write;
