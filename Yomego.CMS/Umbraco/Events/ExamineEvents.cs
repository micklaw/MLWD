        ...
        public void OnApplicationStarted(UmbracoApplicationBase umbracoApplication, ApplicationContext applicationContext)
        {
            ...
            if (!_ran)
            {
                lock (_lockObj)
                {
                    if (!_ran)
                    {
                        ExamineManager.Instance.IndexProviderCollection[Yomego.CMS.Constants.Examine.MainExamineIndexProvider].GatheringNodeData += ExamineEventsGatheringNodeData;
                        _ran = true;
                    }
                }
            }
